...
const checkScheduleBelongsToRestaurantOnCreate = async (value, { req }) => {
  ...
  if (!schedule) {
    return Promise.reject(new Error('The schedule Id does not exist'))
  }
  if (schedule.restaurantId !== req.body.restaurantId) {
    return Promise.reject(new Error('The schedule does not belong to the restaurantId gave'))
  }
...
const checkScheduleBelongsToRestaurantOnUpdate = async (value, { req }) => {
  ...
  if (product.restaurantId !== schedule.restaurantId) {
    return Promise.reject(new Error('The schedule does not belong to the restaurant of this product'))
  }
  return Promise.resolve()
}
